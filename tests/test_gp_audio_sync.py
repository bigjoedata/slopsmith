...
def _make_gp_zip(asset_id="abc-123", ogg_stems=("abc-123",)) -> zipfile.ZipFile:
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w") as zf:
        zf.writestr("Content/score.gpif", _gpif_bytes(asset_id))
        for stem in ogg_stems:
            zf.writestr(f"Content/Assets/{stem}.ogg", b"OggS-fake-audio")
    buf.seek(0)
...
# ── _parse_gpif / _resolve_ogg_asset ────────────────────────────────────────────
...
def test_resolve_ogg_asset_matches_declared_id():
    with _make_gp_zip(asset_id="match", ogg_stems=("other", "match")) as zf:
        stem, path = _resolve_ogg_asset(zf)
        assert stem == "match"
        assert path == "Content/Assets/match.ogg"


def test_resolve_ogg_asset_falls_back_to_first_when_unmatched():
...
def test_resolve_ogg_asset_none_when_no_ogg():
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w") as zf:
